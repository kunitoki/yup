...
- Fixed a data race in `KMeterState`'s per-channel getters (`getPeakLevel`/`getAverageLevel`/`getPeakHoldLevel` with an explicit channel index): they read the plain `ChannelState` floats the audio thread updates, bypassing the atomics the aggregate (channel -1) path uses. The UI-visible per-channel levels are now atomic, and the audio-side ballistics compute on locals and publish once per block
