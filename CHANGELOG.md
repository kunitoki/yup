...
- SDL3 windowing: use the per-window display scale for high-density windows on every platform, and keep logical component bounds separate from pixel framebuffer resize events. This fixes undersized fonts and widgets on Windows high-DPI displays and keeps rendering correct when moving windows between displays.
