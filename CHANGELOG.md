...
- GUI: multitouch input on mobile (iOS, Android) and Emscripten in a mobile browser. Every finger is delivered through the existing mouse callbacks (`mouseDown` / `mouseDrag` / `mouseUp`, left button held): the first finger behaves exactly like a mouse, each additional finger arrives in parallel with a stable, dense finger index exposed as `MouseEvent::isTouch()` / `MouseEvent::getTouchIndex()`, plus touch pressure as `MouseEvent::getPressure()` (0.0-1.0). Each finger is hit-tested independently, keeps its index for the whole contact, has its own double-click detection, and only the first finger takes keyboard focus (secondary touches never steal it mid-gesture). SDL's synthetic touch-to-mouse events are disabled on those platforms so every touch is delivered exactly once.
