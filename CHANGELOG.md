...
- SDL3 windowing: use the per-window display scale for high-density windows on every platform, and keep logical component bounds separate from pixel framebuffer resize events. Coordinate conversions (logical ↔ screen pixel) are now applied at all SDL API boundaries when `SDL_WINDOW_HIGH_PIXEL_DENSITY` is active: window sizing, positioning, resize/move events, mouse input, and DPI-change handling. This fixes undersized fonts and widgets on Windows high-DPI displays (especially remote desktop sessions spanning mixed-DPI screens) and keeps rendering correct when moving windows between displays.
