...
- Added `tests/yup_graphics/yup_WindowPresentParity.cpp`: presentation tests that drive `GraphicsContext` through a full frame and read framebuffer 0, the surface `SDL_GL_SwapWindow` presents. Covers the whole surface receiving the clear, drawn geometry landing in the right place, a second frame presenting completely, the result not being uninitialised memory, and `end()` leaving neither `GL_SCISSOR_TEST` enabled nor a colour channel masked. OpenGL-only, because the Metal context attaches a `CAMetalLayer` to a real view and cannot be driven without a window
- Added `tests/yup_gui/yup_ComponentParity.cpp`: component tree parity tests that drive the real subtree walk through `Component::snapshotToImage`, which needs only a `GraphicsContext` and a non-zero size, so the walk is covered without a window or a swapchain. Covers child placement, clipping to child bounds, sibling order, invisible children, and nested coordinate composition
- Added `tests/yup_graphics/yup_GraphicsParity.cpp`: drawing parity tests that run the same `Graphics` calls on every backend and assert geometry at sample points rather than against a golden image, so antialiasing differences between rasterizers cannot cause a false failure. Covers `fillAll`, rectangle placement and offset, draw order, and the coordinate origin, which is the case that would otherwise go unnoticed because OpenGL framebuffers start bottom-left and Metal textures top-left. Also covers clipping, clip restoration, and translation, which are the primitives the component tree walk relies on, and pins that `GpuCanvas::beginDraw` resets the canvas to transparent black and discards the colour passed to `create`
