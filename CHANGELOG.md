...
- `ShaderTranspiler` now emits WebGL2 / GLES3-compatible GLSL when targeting the `glsl`/`essl` backend: it no longer emits Vulkan GLSL (`#version 450 es`, `gl_VertexIndex`, `layout(set=…)`, separate `texture2D`/`sampler`). ESSL is emitted as `#version 300 es` and image+sampler pairs are folded into combined `sampler2D` uniforms (GLES has no separate sampler objects). This fixes `SpinningCubeDemo` failing to compile its cube and blur shaders on WebGL2/Emscripten (`'450' : client/version number not supported`, `'gl_VertexIndex' : undeclared identifier`).
- OpenGL / OpenGL ES binding correctness: `ShaderReflection` now records the folded combined-sampler uniforms (`glCombinedSamplers`) and their texture units, and `GpuPipeline::compileFromBundle` generates a GL program-link fixup table (`makeGLFixupBlob`, threaded through `GpuShaderSource::glFixup`) so UBO block bindings and sampler texture units resolve by name after linking — GLES 3.00 / WebGL2 cannot express these via `layout(binding=)` qualifiers.
### Graphics RHI
...
- New `makeGLFixupBlob(ShaderReflection)` helper (`rhi/yup_ShaderBindingMap.h`) plus `GpuShaderSource::glFixup` / `glFixupSize` fields: on the OpenGL / OpenGL ES backend, carry the name→slot table that fixes up UBO block bindings and combined-sampler texture units after program linking. Ignored by every non-GL backend.
