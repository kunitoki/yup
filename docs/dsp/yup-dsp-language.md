...
- From **Cmajor**: a readable `processor` surface with typed endpoints
  (`input stream`, `output stream`, `input event`, `output event`, `input parameter`, `output parameter`),
...
Endpoints use `input parameter` and `output parameter` in processors and graphs.
The old `input value` and `output value` spellings are rejected with migration
diagnostics, with no aliases. `value` is an ordinary identifier. Input defaults,
annotations, smoothing, automation, graph forwarding, and host parameter APIs
are unchanged. Output parameters retain meter behavior. Regenerate bundles
after migrating their source.
...
constants (`pi`, `e`), unary `- ~ !`, the arithmetic operators
...
- Integer literals: `0`, `42`, `-7`, `0xff`, `0b1010`; underscores separate digits, as in `1_000`.
- Float literals: `1.0`, `0.5`, `1e-3`, `2.5e+2`.
- Boolean literals: `true`, `false`.
- Strings: `"..."` (metadata and annotations only).

Integer literals preserve their exact signed 64-bit value, including
`-9223372036854775808`. Uncontextualized literals outside the `int32` range
have type `int64`. Out-of-range literals are compile errors. Array sizes,
channel counts, voice counts, rate factors and connection delays are checked
before narrowing to their 32-bit representation; their individual semantic
limits still apply.

Explicit nonfinite constants (`inf`, `nan`, or overflowing floating literals
such as `1e9999`) are compile errors.
...
processor Saturator {
    ...
    input  parameter float drive = 0.5 [[ name: "Drive", min: 0, max: 2 ]];
    output parameter float level;                // meter, written by the kernel
...
| Parameter (read) | `input parameter float name = default;` | Host-automatable value, **sampled once per block**. Element type from the type keyword (`float32` default; `float64`/`int32`/`int64` supported). |
| Meter (write) | `output parameter float name;` | Written by the kernel, read by the host. |
...
```ydsp
input parameter float rate [[ name: "Rate", min: 0.5, max: 12.0, init: 4 ]];
```
...
[2.9](#29-delay-and-smoothing-primitives)) and is legal only on an `input parameter
float` of a processor with a per-sample `process { }` body:

```ydsp
input parameter float cutoff = 1500.0 [[ name: "Cutoff", min: 60.0, max: 12000.0, smoothing: 0.02 ]];
```
...
`input parameter` parameter — of a processor or of a graph — and needs at least two
labels:

```ydsp
input parameter float wave = 0.0 [[ name: "Waveform", min: 0.0, max: 3.0,
...
```ydsp
input parameter float cutoff = 700.0 [[ name: "Cutoff", min: 30.0, max: 12000.0, mid: 632.46 ]];
```
...
[2.11](#211-events) and the request-based `process()` API in
[6](#6-public-api-c)).
...
State arrays, struct-array instances and array fields, and indexed streams
check runtime indices. Out-of-range reads return zero and writes are discarded.
Bounds checks use an unsigned comparison to reject negative and oversized indices
together. Reads from nonempty fixed state arrays select a valid fallback index
before loading, then return zero for invalid indices without a bounds branch.
Unconditional parameter-derived delay clamps can still be evaluated
once per kernel call when later accesses require guards.
Each struct dimension is checked separately before flattening, so indexing past
one instance's field cannot access the next instance. Constant invalid indices
are compilation errors. Stream bounds use the current kernel block length,
including partial blocks. Checks are omitted for proven in-range constants and
simple bounded loop indices, including nonnegative loops ending at
`blockSize - N` for nonnegative `N`; other checked accesses can keep a loop scalar.
The optimizer also removes checks when a scalar `int32` range proves the entire
index range fits the allocation, for example `index & 7` or
`clamp (index, 0, 7)` for an eight-element array. Proofs can flow through integer
min/max, selects, copies and nonoverflowing addition, subtraction and multiplication
(for example `(index & 3) * 4 + 3` in a 16-element array). Mutable values
with multiple definitions generally remain unknown. Sample processors also
recognize persistent counters updated by `wp = wp + 1; if (wp >= N) { wp = 0; }`
when initialization and every other write, including event handlers, preserve
`0..N-1`. This removes checks on ring accesses and on proven wrapped delay taps.
Bounded nonnegative float-to-int conversions used by these proofs map any NaN
reaching the conversion to zero. Unsupported updates and processors with local
functions retain their checks.
Declared state layouts that exceed the compiler's address range are rejected.
...
```ebnf
statement = assignment | if_statement | match_statement | for_statement
...
- `match (expression) { literal => { statements }, _ => { statements } }`.
...
**Parameter writing**: an `output parameter` endpoint may be assigned in either
mode. An `input parameter` endpoint (parameter) may be *read* in both modes but
...
Match statements evaluate an integer (`int32` or `int64`) or boolean selector
once and execute the matching arm, without fallthrough:
...
Patterns are integer literals (including negative values) or `true`/`false`,
compatible with the selector type. Arms require braces and commas between them;
a trailing comma is allowed. Each arm introduces its own local scope, using the
same rules as `if`/`else`: sibling arms may reuse a local name, but an outer
local cannot be redeclared. Duplicate
patterns are errors. The optional `_` fallback must be last; without it, an
unmatched selector does nothing. Matches lower to ordinary `if`/`else if`/`else`
branches and participate in the same optimizations. A compile-time constant
selector eliminates the other arms, leaving only the selected block. Boolean
patterns use the predicate directly (`true`) or its negation (`false`), so they
follow the same optimizer and vectorizer paths as equivalent `if` conditions.
Vectorization still depends on the operations and control flow in the arms. A block requires at least one arm and supports
up to 128 literal arms. Matches can nest and can appear in process, init, event,
and function bodies. They are statements, not expressions; ranges, guards,
floating-point patterns, named constants, and destructuring are not supported.
...
- Arithmetic: `+ - * / %` (float and int), unary `-`, logical `&& || !`
  ...
  `&&` evaluates its right operand only when the left is true; `||` only
  when the left is false. A ternary evaluates only its selected branch.
  `select(cond,a,b)` evaluates both arguments. These rules also hold with
  `fastMath` enabled. Short-circuit and memory-guard branches retain conditional
  evaluation. Unrelated branches may still optimize, and unconditional
  loop-invariant work can move out of the sample loop; optimization reports
  describe the remaining restrictions.
...
cast explicitly instead. Integer `abs(INT_MIN)` and negation of `INT_MIN`
saturate to `INT_MAX` for the operand width. Integer kernel division truncates
toward zero; `INT_MIN / -1` saturates to `INT_MAX`, with remainder zero.
Division and remainder by zero return zero. These rules also apply when the
kernel optimizer folds these operations, regardless of `fastMath`.

Integer kernel addition, subtraction and multiplication saturate on overflow to
the signed minimum or maximum of the operand width, independently of `fastMath`.
Constant folding follows the same rule. Proven nonoverflowing operations retain
direct arithmetic instructions, including validated ring-counter updates.

Algorithms that require modular arithmetic, such as integer noise generators,
must preserve wraparound explicitly. An `int64` to `int32` cast keeps the low
32 bits rather than saturating. For an `int32` random state, the following
computes the intermediate without overflow and retains the original modulo-2^32
sequence:

```ydsp
rng = int32 (int64 (rng) * 1664525 + 1013904223);
```

Integer kernel shifts use the low five bits of the count for `int32`, or the
low six bits for `int64`, including negative counts. Left shifts discard bits
beyond the operand width; right shifts preserve the sign. Constant folding
uses the same rules as native and WebAssembly execution.

Float-to-integer conversions truncate toward zero, saturate to the destination
integer limits on overflow (including infinities), and return zero for NaN.
These rules apply to both float widths and both integer widths, including
implicit conversions and constant folding, independently of `fastMath`.
...
`[[ smoothing: t ]]` on an `input parameter float` (see [2.1](#21-endpoints)) is
...
```ydsp
input parameter float cutoff = 1500.0 [[ smoothing: 0.02 ]];
process { out = in * cutoff; }
```

```ydsp
input parameter float cutoff = 1500.0;
process {
...
endpoint_declaration = ("input" | "output") "stream"
    type identifier [ "[" integer "]" ] [ annotations ] ";"
    | "input" "parameter" type identifier [ "=" expression ] [ annotations ] ";"
    | "output" "parameter" type identifier [ annotations ] ";"
...
A bank may also sit on a **graph** instantiation, which replicates the whole
chain rather than one node:
...
graph VoiceChain {
    ...
    connection { midi -> osc.midi; osc.out -> flt.in; flt.out -> dly.in; dly.out -> out; }
}
...
graph Patch [[ main ]] {
    ...
    node v = VoiceChain[8] [[ mode: poly, stealing: oldest ]];

    connection { midi -> v.midi; v.out -> out; }
}
```

Each of the eight voices now runs the whole `VoiceChain` - its own oscillator,
filter and delay - so two simultaneous notes never share effect state. The
runtime keeps one kernel per member and N state slices; only the *unit* the
voice loop wraps around changes (voice-major over the group's members, with
intra-group delay rings strided per voice). A banked graph must declare at
least one `input event` and exactly one float32 output stream, and contain at
least one member that declares an event handler. Members must be ordinary
single-voice float32 processors: nested voice banks, per-member
over/undersampling and midi-only members are all rejected. Annotations are
accepted on the banked instantiation only (`mode: mono` is still rejected on
`[N > 1]`, and a `[1]` or bare instantiation behaves exactly like a plain
subgraph - bit-identical output to writing the chain inline).

Because one note event is allocated once per group and then fanned out to
*every* member that subscribes to the event, two subscribing members of one
chain always handle a given note on the same voice.

Adjacent members of one chain that are pure per-sample processors (no events,
no activity flag, one stream in and out, no rate change) are fused by the
optimiser into a single kernel, exactly as plain chains are - the fused member
still runs once per voice under the group's slot table, so per-voice state and
parameter behaviour are unchanged while the intra-group junction stops being a
per-voice scratch round-trip. Fusion never crosses a group boundary.

`examples/graphics/data/synths/PerVoiceEcho.ydsp` is the worked example: eight
sine voices, each with its own feedback delay.
...
- **The flag must be a scalar `int` with no initialiser** ("starts at 0,
  ...
  unknown `role:` value, a non-`int` or array-typed flag, a second flag or an
  initialiser are all
...
an unknown node name. After inlining, a banked chain leaves no node named after
the bank, so the query also accepts the group name (`getActiveVoiceCount
("v")`); a chain member carrying an activity flag drives the predicate, and a
chain whose members all lack one reports the number of held voices instead.

In a banked chain the unit of skipping is the whole voice, not the member. A
voice runs while any member is awake or held (or has a pending event, or a
block-wide automation/all-sound-off timeline); it is skipped - all members
frozen, per-voice delay rings included - only when *every* member reports its
`voiceActivity` flag asleep. That makes the effect members' flags load-bearing:
a delay or reverb that wants its voice skipped must keep its flag set until its
own tail has genuinely died (flag 0 asserts "nothing audible remains and
nothing revives me on its own", the usual contract). An effect member without a
flag never sleeps, so a chain only skips when every one of its members opts in
- fused effect members (which cannot carry a flag) therefore keep their voice
running. `getActiveVoiceCount` follows the same predicate: it reports the voice
count of the first flag-carrying chain member, or the held count when no member
carries a flag - do not assert `== 1` after one note on a chain whose members
carry no flag.
...
Sample-accurate automation currently requires base-rate nodes. Query
`supportsSampleAccurateAutomation(slot)` before scheduling: if a slot drives
any oversampled or undersampled node, `process()` rejects the entire request
with `unsupportedAutomation`, including automation at offset zero. Use the
block-boundary parameter setters for those nodes instead; these do not promise
sample-accurate timing through the resampler's buffered history.

Dispatch uses ordered cursors rather than rescanning every queue at every split.
Node-wide automation is ordered once and reused across voices. At equal offsets,
automation runs first, then all-sound-off, then handlers; automation and handler
entries retain their arrival order within each queue.

Latency-compensated output events remain pending across as many blocks as needed,
including when the host changes block sizes. Blocks cover offsets `[0, numSamples)`:
an event exactly at the end belongs to offset zero of the next block, both for
node routing and host MIDI output. Pending events use the capacity reserved by
`prepare()`; overflow increments `getDroppedOutputEventCount()`, and `reset()`
discards them.
...
graph MyPatch {
    ...
    input  parameter float master = 0.8;    // patch-level parameter
...
graph MasterBus {                            // a subgraph: no [[ main ]]
    input  stream in;
    output stream out;
    input  parameter float threshold = -18.0;
    input  parameter float reverbMix = 0.25;
...
- A subgraph's `input parameter` forwards to every node parameter it drives: the
  ...
  `output parameter` forwards outwards the same way.
...
Because the mechanism is inlining rather than runtime nesting, the runtime
mechanisms that remain unsupported on a subgraph node are rejected, each with
an explicit diagnostic:

| Rejected | Why |
|----------|-----|
| `node v = Chain[8];` where `Chain` has no `input event` | a banked graph must be event-driven: a bank replicates state per voice, and only events create voices |
| `node v = Chain[8];` where `Chain` does not declare exactly one float32 output stream | the group's per-voice summing path needs one float32 output to sum into |
| `node v = Chain[8];` where no member declares an event handler | nothing in the chain would ever respond to a note |
| `node v = Chain[8];` with `[[ mode: mono ]]` | mono needs exactly one voice; use a single-voice processor instead |
| `node v = Outer[8];` where `Outer` itself contains a banked graph | nested voice banks are not supported (a bank inside a bank would need two-level voice state) |
| a member of a banked graph that is itself banked, rate-changed, midi-only, or declares a non-float32 stream | the group runs one kernel per member under one shared slot table; all of these need machinery the group does not have |
| `node g = Sub * 4;` | over/undersampling is a per-processor rate change |
| `node g = Sub;` where `Sub` declares `input event midi` | a plain (single-voice) instantiation of an event-input graph is still rejected; only a banked instantiation may receive events |
...
For benchmark assembly listings, set `YUP_DUMP_KERNEL=1` (or `*`) when running
`YdspBenchmarkTests`. To limit listings, use comma-separated kernel labels such
as `YUP_DUMP_KERNEL=reverb,compressor`. An unset, empty or `0` value disables
printing. Listings and instruction histograms are printed outside timed runs.
...
a member's meter wired to a graph `output parameter` still reports through it. State,
...
- Stream types: per-sample scalar in sample mode, array of `blockSize`
  ...
  (`float32` default).
- Endpoint kinds: `input stream` / `output stream` / `input parameter` /
  `output parameter`; value endpoints may be any primitive type.
...
### Tracing scalar values

Use `trace("env={env}, note={note}");` inside a process, init, event handler,
or function body. Each `{name}` captures a scalar variable at that statement;
integers retain all 64 bits, and booleans print as `1` or `0`. Up to eight
placeholders are allowed. Use `{{` and `}}` for literal braces. Expressions,
field/index access and format specifiers inside placeholders are not supported;
assign their value to a local variable first. Invalid strings and unresolved
names produce compile errors even when tracing is disabled.

Tracing is disabled by default. Set `YdspCompileOptions::enableTracing` (or
`YdspBundleCompileOptions::enableTracing`) before compiling to retain it. When
disabled, no trace recording instructions or queue storage are generated.

```cpp
yup::YdspCompileOptions options;
options.enableTracing = true;
auto result = compiler.compile (source, options);
// Check result, prepare the graph, then start audio as usual.

// From a control-thread timer, never the audio callback:
graph.printTraceMessages();
// Alternatively, drainTraceMessages() returns strings for a UI or custom logger.

// Pause/resume recording without recompilation:
graph.setTracingEnabled (false);
```

Enabled kernels only copy scalar values into a preallocated queue. Formatting,
string allocation and console I/O happen when the host drains it. There is no
background printing thread: the host must call the drain/print API periodically.
The queue holds 256 complete messages; overflow drops new messages and increments
`getDroppedTraceCount()`. Messages from init, event handlers and processing share
execution order. `prepare()` clears the queue and drop count; pausing recording
keeps queued messages available. Instrumented kernels may run slower and traced
loops stay scalar, so disable tracing for performance measurements.

Use one processing producer and one draining consumer. Stop both before
preparing, resetting, moving or destroying the graph. Never call
`drainTraceMessages()` or `printTraceMessages()` on the audio thread.
...
struct YdspKernelContext {
    ...
    void*        params;     // input parameter endpoint values (sampled once per block)
    void*        paramOut;   // output parameter endpoint values
...
if (result.wasOk())
{
    ...
    graph.process (yup::YdspProcessRequest { inputs, outputs, 512 });
...
- `YdspCompiler::compile(source, options, importBasePath = {}, threadPool =
  ...
  portability checks. `fastMath` defaults to true on native targets, where the
  compiler contracts `a * b + c` and lowers widened transcendentals to SLEEF's
  fast `u35` tier (scalar float32 values always stay on the platform libm);
  the wasm backend keeps strict numerics regardless of the flag. Set it to
  false for strict 1-ULP native behavior (SLEEF `u10` widened values, no
  contraction). Set `emitOptimizationReport` to inspect the selected ISA, lane
  width, rejected transforms, generated code size and compile time with
  `YdspCompiler::getOptimizationReport()`.
- `YdspAudioGraph::prepare(sampleRate, maxBlockSize, maxEventsPerVoicePerBlock =
  64, maxAutomationPerNodePerBlock = 32, maxOutputEventsPerBlock = 32)`
  returns `Result` and preallocates state, buffers and parameter storage.
  It must succeed before `process`. Sample rates must be finite, positive and
  representable as float at every node's rate; block sizes and capacities must
  be positive and buffer-size arithmetic must fit. Validation failures preserve
  the previous preparation. Each distinct event
...
- `YdspAudioGraph::process(const YdspProcessRequest&)` is the only processing
  entry point. The request contains typed input/output spans, a sample count,
  a span of MIDI-buffer pointers in event-input declaration order, an automation
  span and optional MIDI output. A shorter event-input span or a null entry
  carries no events for that input. Each input is routed only to its explicitly
  connected nodes. Stream type/count mismatches return a `YdspProcessResult`
...
- `YdspAudioGraph::getParameter(name)` / `setParameter(name, value)` - typed
  parameter access, with double/int variants. Setters share a bounded
  single-producer queue: use one control thread or serialize producers.
  Only `process()` consumes updates, taking a fixed queue-head snapshot per block.
  Overflow drops the new update and increments `getDroppedEventCount()`.
- Parameter getters and `getOutputValue(name)` meter getters read independent,
  atomic values published at the end of a processed block. They may run
  concurrently with `process()`; reading several slots is not a coherent
  whole-graph snapshot. Before processing, parameters expose compiled defaults.
  A setter followed immediately by a getter does **not** flush the update.
  Zero-length processing does not flush updates either.
- Resolve `getParameterSlot(name)` / `getOutputValueSlot(name)` once and use the
  typed slot-based setters/getters to avoid name lookup and allocation. Lifecycle
  operations (`prepare()`, `reset()`, moving or destroying the graph) must not
  overlap processing or control access.
- `YdspAudioGraph::getExecutionReport()` — the optimiser's worst-case report
  (see below).

The processing API is `process(const YdspProcessRequest&)`. The
request groups input/output spans, sample count, per-input MIDI spans, an
automation span and optional MIDI output. Positional overloads have been removed.
All validation happens before consuming parameters or modifying state, queues,
counters or outputs. In particular:

- Negative sample counts and null storage for non-empty spans are rejected.
- Invalid automation slots, non-float32 targets and offsets outside
  `[0, numSamples)` reject the whole block instead of being clamped or dropped.
- MIDI timestamps must also lie inside the block; MIDI output cannot alias an
  input MIDI buffer.
- Zero-length calls still require preparation and correctly typed stream
  entries, with no MIDI/automation events. Valid zero-length calls do nothing.
- Capacity exhaustion during a valid block retains the existing drop-and-count
  behavior; it is not a request-validation error.

Slot access uses explicit names such as `setParameterBySlot(slot, value)`,
`getDoubleParameterBySlot(slot)` and `getOutputValueBySlot(slot)`. Named
parameter methods continue accepting string literals, `String` and `StringRef`
without integer-overload ambiguity.
...
- Mono streams only (the ABI already passes `float*` arrays; multichannel is a
  ...
  (MIDI-only) is exempt.
- A subgraph that declares any `input event` can be instantiated as a node
  only when the instantiation is a voice bank (`Sub[4]`); a plain
  single-voice instantiation is rejected, because graph-scope event handling
  is what the bank's group-level routing is built on. The bank itself must
  expose the event input, one float32 output stream and at least one
  event-handler member.
- Nested voice banks (a banked graph inside another banked graph) are not
  supported in v1; a banked graph's members must be ordinary single-voice
  float32 processors with no per-member rate change.
- `output event` edges out of a group member are not supported yet: the
  per-node output event queue carries no voice identity, which is a Phase 2
  concern. Per-voice meters (`output parameter` read inside a banked chain's
  members) also have no defined meaning - every voice writes the same slot and
  the last voice wins.
...
- Idle voices are only skipped when the processor opts in with a
  ...
  runs — and is decided per block for the whole kernel, never per sample. In a
  banked chain the skipped unit is the whole voice and only when *every*
  member opts in (a flag is allowed on effect processors too); fused effect
  members cannot carry a flag, so their voices always run.
...
Diagnostics print `path:line:column: severity: message`, followed by two source
lines before the issue and two after it. The error is the third source line
when that context exists; excerpts are shortened at file boundaries. A caret
marks the offending column and tildes underline the rest of its source range.
Tabs are preserved so the marker aligns with the source.

Pass the source file path as the compiler's `importBasePath` argument to identify
the root file. Sources compiled with only a directory or no path are named
`<memory>`. Imported files retain their own paths and source excerpts.

Diagnostic consumers should use `YdspDiagnostic::range`, whose positions are
one-based and whose end is exclusive. The range also carries the source identity.
