...
def get_logo_image() -> yup.Image:
    return yup.ImageCache.getFromFile(this_file.getSiblingFile("data").getChildFile(f"logo.png"))

def get_original_image_path(test_name: str) -> yup.File:
    return this_file.getParentDirectory().getSiblingFile("runtime_data").getChildFile(f"{test_name}.png")

def get_reference_image_path(test_name: str) -> yup.File:
    return this_file.getSiblingFile("reference_images").getChildFile(f"{test_name}.png")

def get_failed_folder_path() -> yup.File:
    return this_file.getParentDirectory().getSiblingFile("compare_data")

def get_failed_generated_image_path(test_name: str) -> yup.File:
    return get_failed_folder_path().getChildFile(f"{test_name}_generated.png")

def get_failed_reference_image_path(test_name: str) -> yup.File:
    return get_failed_folder_path().getChildFile(f"{test_name}_reference.png")

def get_failed_diff_image_path(test_name: str) -> yup.File:
    return get_failed_folder_path().getChildFile(f"{test_name}_diff.png")

#==================================================================================================

def load_image(file: yup.File) -> yup.Image:
    return yup.ImageCache.getFromFile(file)

def store_image(file: yup.File, img: yup.Image):
    file.deleteFile()
    assert yup.PNGImageFormat().writeImageToStream(img, yup.FileOutputStream(file))
...
def is_image_equal_reference(update_rendering: bool, original_image: yup.Image, threshold = 0.0) -> bool:
