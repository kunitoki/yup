...
def get_big_random(r: yup.Random) -> yup.BigInteger:
    b = yup.BigInteger()
...
def test_construct_with_int64():
    big_int = yup.BigInteger(1234567890123)
    assert big_int.toInt64() == 1234567890123

#==================================================================================================

def test_copy_constructor():
    original = yup.BigInteger(12345)
    copy = yup.BigInteger(original)
...
def test_swap_with():
    a = yup.BigInteger(1234)
    b = yup.BigInteger(5678)
    a.swapWith(b)
...
def test_clear_bit():
    big_int = yup.BigInteger(3)  # binary 11
    big_int.clearBit(0)
...
def test_insert_bit():
    big_int = yup.BigInteger(2)  # binary 10
...
def test_get_bit_range():
    big_int = yup.BigInteger(15)  # binary 1111
...
def test_get_bit_range_as_int():
    big_int = yup.BigInteger(15)  # 1111 in binary
...
def test_shift_bits():
    big_int = yup.BigInteger(1)  # binary 1
...
def test_bitwise_and():
    a = yup.BigInteger(6)  # binary 110
    b = yup.BigInteger(3)  # binary 011
    a &= b
...
def test_equality():
    a = yup.BigInteger(123)
    b = yup.BigInteger(123)
    assert a == b
...
def test_divide_by():
    divisor = yup.BigInteger(3)
    remainder = yup.BigInteger()
    a = yup.BigInteger(10)
...
def test_exponent_modulo():
    a = yup.BigInteger(2)
    exponent = yup.BigInteger(3)
    modulus = yup.BigInteger(5)
...
def test_montgomery_multiplication():
    a = yup.BigInteger(10001)
    other = yup.BigInteger(3)
    modulus = yup.BigInteger(5)
    modulusp = yup.BigInteger(2)

    a.montgomeryMultiplication(other, modulus, modulusp, 1)

    result = yup.BigInteger()
...
def test_extended_euclidean():
    a = yup.BigInteger(240)
    b = yup.BigInteger(46)

    xOut = yup.BigInteger()
    yOut = yup.BigInteger()
    gcd = yup.BigInteger(a)

    gcd.extendedEuclidean(a, b, xOut, yOut)

    assert gcd == yup.BigInteger(2)

#==================================================================================================

def test_comparisons():
    r = yup.Random.getSystemRandom()

    assert yup.BigInteger().isZero()
    assert yup.BigInteger(1).isOne()

    for _ in range(1000, 0, -1):
        b1 = yup.BigInteger(get_big_random(r))
        b2 = yup.BigInteger(get_big_random(r))

        b3: yup.BigInteger = b1 + b2
        assert b3 > b1 and b3 > b2
        assert b3 - b1 == b2
        assert b3 - b2 == b1

        b4: yup.BigInteger = b1 * b2
...
def test_bit_setting():
    ...
    for _ in range(1000, 0, -1):
        ...
        old1 = yup.readLittleEndianBitsInBuffer (test, offset - 6, 6)
        old2 = yup.readLittleEndianBitsInBuffer (test, offset + num, 6)
        yup.writeLittleEndianBitsInBuffer (test, offset, num, value.value)
        result = yup.readLittleEndianBitsInBuffer (test, offset, num)

        assert c_uint32(result).value == value.value
        assert old1 == yup.readLittleEndianBitsInBuffer (test, offset - 6, 6)
        assert old2 == yup.readLittleEndianBitsInBuffer (test, offset + num, 6)
