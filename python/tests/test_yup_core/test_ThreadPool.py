...
class ThreadPoolJob(yup.ThreadPoolJob):
    runCount = 0
    maxRuns = 1
    ...
    def runJob(self):
        ...
        if self.runCount >= self.maxRuns:
            return yup.ThreadPoolJob.JobStatus.jobHasFinished
        else:
            return yup.ThreadPoolJob.JobStatus.jobNeedsRunningAgain
...
def test_threads_constructor():
    ...
    assert pool.getNamesOfAllJobs(False) == yup.StringArray(["abc", "123"])
