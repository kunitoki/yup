...
class Thread(yup.Thread):
    ...
    def run(self):
        self.threadID = yup.Thread.getCurrentThreadId()
        ...
        assert yup.Thread.currentThreadShouldExit()

        self.notify()

class ThreadListener(yup.Thread.Listener):
...
def test_construct_and_start_stop():
    threadID = yup.Thread.getCurrentThreadId()
...
def test_construct_and_start_signal_stop():
    ...
    a.startRealtimeThread(yup.Thread.RealtimeOptions()
        .withProcessingTimeMs(100)
        .withPriority(yup.Thread.Priority.highest))
...
def test_launch():
    launched = False
    start_event = yup.WaitableEvent()
    stop_event = yup.WaitableEvent()
    ...
    assert yup.Thread.launch(runThread)
