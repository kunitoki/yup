...
@pytest.fixture
def zip_file():
    yield yup.File(this_file.getSiblingFile("data").getChildFile("archive.zip"))

@pytest.fixture
def text_file():
    yield yup.File(this_file.getSiblingFile("data").getChildFile("somefile.txt"))

@pytest.fixture
def image_file():
    yield yup.File(this_file.getSiblingFile("data").getChildFile("pysound.jpg"))

#==================================================================================================

def test_constructor_with_file(zip_file):
    z = yup.ZipFile(zip_file)
    assert z.getNumEntries() == 2

#==================================================================================================

def test_constructor_with_nonexisting_file():
    z = yup.ZipFile(this_file.getChildFile("nonexisting.zip"))
...
def test_constructor_with_input_stream(zip_file):
    with open(zip_file.getFullPathName(), "rb") as file:
        mb = yup.MemoryBlock(file.read())
        input_stream = yup.MemoryInputStream(mb, False)
        zip_file = yup.ZipFile(input_stream)
        assert zip_file.getNumEntries() == 2

#==================================================================================================

def test_constructor_with_input_source(zip_file):
    input_source = yup.FileInputSource(zip_file, False)
    zip_file = yup.ZipFile(input_source)
    assert zip_file.getNumEntries() == 2

#==================================================================================================

def test_get_entry_by_index(zip_file):
    zip_file = yup.ZipFile(zip_file)
    ...
    #assert entry.fileTime == yup.Time.fromISO8601("20240209T183446.000+0100")
    assert entry.fileTime > yup.Time()
    ...
    #assert entry.fileTime == yup.Time.fromISO8601("20240215T083304.000+0100")
    assert entry.fileTime > yup.Time()
...
def test_uncompress_to_nonexistent(zip_file):
    ...
    result = yup.ZipFile(zip_file).uncompressTo(target_dir)
    assert result.wasOk()
...
def test_uncompress_entry(zip_file):
    ...
    result = yup.ZipFile(zip_file).uncompressEntry(0, target_dir)
    assert result.wasOk()
    assert target_dir.getChildFile("somefile.txt").existsAsFile()
...
def test_builder_add_stream_entry(text_file):
    ...
    for level in range(0, 9):
        builder = yup.ZipFile.Builder()
        input_stream = yup.FileInputStream(text_file)
        builder.addEntry(input_stream, level, text_file.getFileName(), yup.Time.getCurrentTime())
        assert builder.writeToStream(output.createOutputStream())
        assert output.existsAsFile()
        ...
        assert z.getNumEntries() == 1
        assert z.getEntry(0).filename == text_file.getFileName()
        assert z.getEntry(0).fileTime > yup.Time()
