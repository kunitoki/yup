...
public:
    ...
    {
        handle native_enum = global_internals_native_enum_type_map_get_item (std::type_index (typeid (EnumType)));
        if (native_enum)
        {
...
{
    static constexpr bool value = std::is_enum<T>::value && type_caster_enum_type_enabled<T>::value;
};
...
protected:
    ...
    void check_holder_compat()
    {
        ...
        bool inst_has_unique_ptr_holder = (typeinfo->holder_enum_v == holder_enum_t::std_unique_ptr);
...
public:
    ...
    std::weak_ptr<type> potentially_slicing_weak_ptr()
    {
        if (typeinfo->holder_enum_v == detail::holder_enum_t::smart_holder)
        {
            ...
            shared_ptr_storage = sh_load_helper.load_as_shared_ptr (typeinfo,
                                                                    value,
    ...
    bool try_implicit_casts (handle src, bool convert)
    {
        for (auto& cast : typeinfo->implicit_casts)
        {
            ...
            if (sub_caster.load (src, convert))
            {
                ...
                {
                    shared_ptr_storage = std::shared_ptr<type> (sub_caster.shared_ptr_storage, (type*) value);
                }
                return true;
            }
        }
...
public:
    ...
    template <typename T_>
    using cast_op_type = conditional_t<std::is_same<typename std::remove_volatile<T_>::type,
...
struct pyobject_caster
{
    ...
    pyobject_caster()
        : value (reinterpret_steal<type> (handle()))
    {
    }
...
template <typename type>
using cast_is_temporary_value_reference = bool_constant<(std::is_reference<type>::value || std::is_pointer<type>::value)
