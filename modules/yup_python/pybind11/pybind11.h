...
protected:
    ...
    void initialize (Func&& f, Return (*) (Args...), const Extra&... extra)
    {
        ...
        constexpr bool is_function_ptr = std::is_convertible<Func, FunctionType>::value && sizeof (capture) == sizeof (void*);
                             ...
                             size_t args)
    {
        ...
        std::string signature = detail::generate_function_signature (text, rec, types, type_index, arg_index);
        ...
        if (! chain)
        {
            ...
            ((detail::function_record_PyObject*) py_func_rec.ptr())->cpp_func_rec = unique_rec.release();
        ...
        else
        {
            ...
            if (rec->prepend)
            {
                ...
                chain_start = rec;
                rec->next = chain;
                auto* py_func_rec = (detail::function_record_PyObject*) PyCFunction_GET_SELF (m_ptr);
...
{
public:
    explicit mod_gil_not_used (bool flag = true)
        : flag_ (flag)
    {
    }
...
public:
    ...
    explicit constexpr multiple_interpreters (level l)
        : level_ (l)
    {
    }
...
*/
inline PyObject* get_cached_module (const pybind11::str& nameobj)
{
...
*/
inline void cache_completed_module (const pybind11::object& mod)
{
    dict state = detail::get_python_state_dict();
    if (! state.contains ("__pybind11_module_cache"))
    {
...
struct property_cpp_function_classic
{
    ...
    {
        return cpp_function ([pm] (const T& c) -> const D&
        {
            return c.*pm;
        },
                             is_method (hdl));
    ...
    {
        return cpp_function ([pm] (T& c, const D& value)
        {
            c.*pm = value;
        },
                             is_method (hdl));
...
struct property_cpp_function_sh_raw_ptr_member
{
    ...
    static cpp_function readonly (PM pm, const handle& hdl)
    {
        ...
        if (tinfo->holder_enum_v == holder_enum_t::smart_holder)
        {
            ...
            {
                std::shared_ptr<T> c_sp = type_caster<std::shared_ptr<T>>::shared_ptr_with_responsible_parent (
                    c_hdl);
                D ptr = (*c_sp).*pm;
    ...
    static cpp_function write (PM pm, const handle& hdl)
    {
        ...
        if (tinfo->holder_enum_v == holder_enum_t::smart_holder)
        {
            return cpp_function ([pm] (T& c, D value)
            {
                c.*pm = std::forward<D> (std::move (value));
            },
                                 is_method (hdl));
        }
...
public:
    ...
    {
        cpp_function fget ([pm] (const object&) -> const D&
        {
            return *pm;
        },
                           scope (*this)),
...
private:
    ...
    static void init_instance (detail::instance* inst, const void* holder_const_void_ptr)
    {
        ...
        inst->is_alias = detail::dynamic_raw_ptr_cast_if_possible<type_alias> (value_ptr_w_t) != nullptr;
        if (holder_void_ptr)
        {
            // Note: inst->owned ignored.
...
struct enum_base
{
    ...
    PYBIND11_NOINLINE void init (bool is_arithmetic, bool is_convertible)
    {
        ...
        m_base.attr ("name") = property (cpp_function (&enum_name, name ("name"), is_method (m_base), pos_only()));
    ...
    m_base.attr (op) = cpp_function (            \
        ...
        if (is_convertible)
        {
            ...
            if (is_arithmetic)
            {
                ...
                m_base.attr ("__invert__") = cpp_function ([] (const object& arg)
                {
                    return ~(int_ (arg));
                },
                                                           name ("__invert__"),
...
#undef PYBIND11_ENUM_OP_STRICT
        ...
        m_base.attr ("__getstate__") = cpp_function ([] (const object& arg)
        {
            return int_ (arg);
        },
                                                     name ("__getstate__"),
        ...
        m_base.attr ("__hash__") = cpp_function ([] (const object& arg)
        {
            return int_ (arg);
        },
                                                 name ("__hash__"),
    ...
    PYBIND11_NOINLINE void value (const char* name_, object value, const char* doc = nullptr)
    {
...
void implicitly_convertible()
{
    ...
    struct set_flag
    {
        ...
        explicit set_flag (thread_specific_storage<int>& flag_)
            : flag (flag_)
        {
