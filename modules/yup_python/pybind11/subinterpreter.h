...
{
public:
    explicit subinterpreter_scoped_activate (const subinterpreter& si);
    ~subinterpreter_scoped_activate();

    subinterpreter_scoped_activate (subinterpreter_scoped_activate&&) = delete;
    subinterpreter_scoped_activate (const subinterpreter_scoped_activate&) = delete;
    subinterpreter_scoped_activate& operator= (subinterpreter_scoped_activate&) = delete;
    subinterpreter_scoped_activate& operator= (const subinterpreter_scoped_activate&) = delete;
...
public:
    ...
    subinterpreter (const subinterpreter& copy) = delete;
    subinterpreter& operator= (const subinterpreter& copy) = delete;

    subinterpreter (subinterpreter&& old) noexcept
        : istate_ (old.istate_)
        , creation_tstate_ (old.creation_tstate_)
    {
        old.istate_ = nullptr;
    ...
    static inline subinterpreter create (const PyInterpreterConfig& cfg)
    {
...
public:
    scoped_subinterpreter()
        : si_ (subinterpreter::create())
        , scope_ (si_)
    {
    }

    explicit scoped_subinterpreter (const PyInterpreterConfig& cfg)
        : si_ (subinterpreter::create (cfg))
        , scope_ (si_)
    {
    }
...
inline subinterpreter_scoped_activate::subinterpreter_scoped_activate (const subinterpreter& si)
{
    if (! si.istate_)
    {
