...
struct npy_api
{
    ...
    enum constants
    {
        ...
        NPY_INT32_ = platform_lookup < std::int32_t,
        long,
        int,
        short > (NPY_LONG_, NPY_INT_, NPY_SHORT_),
        NPY_UINT32_ = platform_lookup < std::uint32_t,
        unsigned long,
        unsigned int,
        unsigned short > (NPY_ULONG_, NPY_UINT_, NPY_USHORT_),
        NPY_INT64_ = platform_lookup < std::int64_t,
        long,
        long long,
        int > (NPY_LONG_, NPY_LONGLONG_, NPY_INT_),
        NPY_UINT64_ = platform_lookup < std::uint64_t,
        unsigned long,
        unsigned long long,
        unsigned int > (NPY_ULONG_, NPY_ULONGLONG_, NPY_UINT_),
        NPY_FLOAT32_ = platform_lookup < float,
        double,
        float,
        long double > (NPY_DOUBLE_, NPY_FLOAT_, NPY_LONGDOUBLE_),
        NPY_FLOAT64_ = platform_lookup < double,
        double,
        float,
        long double > (NPY_DOUBLE_, NPY_FLOAT_, NPY_LONGDOUBLE_),
...
template <typename T>
using is_pod_struct = all_of<std::is_standard_layout<T>, // since we're accessing directly in memory
...
struct numpy_scalar
{
    ...
    explicit numpy_scalar (value_type value)
        : value (value)
    {
    }
...
public:
    ...
    explicit dtype (const pybind11::str& format)
        : dtype (from_args (format))
    {
    }

    explicit dtype (const std::string& format)
        : dtype (pybind11::str (format))
    {
    }

    explicit dtype (const char* format)
        : dtype (pybind11::str (format))
    {
    }
...
public:
    ...
    array()
        : array (0, static_cast<const double*> (nullptr))
    {
    }
    ...
    template <typename T,
              typename = detail::enable_if_t<std::is_integral<T>::value && ! std::is_same<bool, T>::value>>
    ...
    {
        detail::npy_api::PyArray_Dims d = { // Use reinterpret_cast for PyPy on Windows (remove if fixed, checked on 7.3.1)
...
public:
    ...
    array_t (const object& o)
        : array (raw_array_t (o.ptr()), stolen_t {})
    {
        if (! m_ptr)
        {
            throw error_already_set();
                                                  ...
                                                  bool (*direct_converter) (PyObject*, void*&))
{
    ...
    auto* dtype_ptr = pybind11::dtype (std::move (names), std::move (formats), std::move (offsets), itemsize)
                          .release()
                          .ptr();
...
struct vectorize_arg
{
    ...
    static constexpr bool vectorize = satisfies_any_of<call_type, std::is_arithmetic, is_complex, is_pod>::value
...
{
    static constexpr auto name = io_name ("typing.Annotated[numpy.typing.ArrayLike, ", "numpy.typing.NDArray[")
