...
using vector_needs_copy = negation<std::is_same<decltype (std::declval<Vector>()[typename Vector::size_type()]),
...
template <typename Vector, typename Class_>
auto vector_if_insertion_operator (Class_& cl, const std::string& name)
...
class_<Vector, holder_type> bind_vector (handle scope, const std::string& name, Args&&... args)
{
...
str format_message_key_error (const KeyType& key)
{
    ...
    {
        do
        { // Trick to avoid "empty catch" warning/error.
        } while (false);
    }
