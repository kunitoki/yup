...
PYBIND11_NOINLINE void all_type_info_populate (PyTypeObject* t, std::vector<type_info*>& bases)
{
    ...
    const auto& type_dict = get_internals().registered_types_py;
...
public:
    explicit values_and_holders (instance* inst)
        : inst { inst }
        , tinfo (all_type_info (Py_TYPE (inst)))
    {
    }

    explicit values_and_holders (PyObject* obj)
        : inst { nullptr }
        , tinfo (all_type_info (Py_TYPE (obj)))
    {
        if (! tinfo.empty())
        {
...
template <typename T, typename D>
handle smart_holder_from_unique_ptr (std::unique_ptr<const T, D>&& src,
                                     return_value_policy policy,
                                     handle parent,
                                     const std::pair<const void*, const type_info*>& st)
{
                                     ...
                                     const std::pair<const void*, const type_info*>& st)
{
    ...
    auto smhldr = smart_holder::from_shared_ptr (std::shared_ptr<void> (src, const_cast<void*> (st.first)));
    tinfo->init_instance (inst_raw_ptr, static_cast<const void*> (&smhldr));
...
template <typename T>
handle smart_holder_from_shared_ptr (const std::shared_ptr<const T>& src,
                                     return_value_policy policy,
                                     handle parent,
                                     const std::pair<const void*, const type_info*>& st)
{
...
struct shared_ptr_parent_life_support
{
    ...
    explicit shared_ptr_parent_life_support (PyObject* parent)
        : parent { parent }
    {
        Py_INCREF (parent);
    }
...
struct load_helper : value_and_holder_helper
{
                                              ...
                                              const char* context = "load_as_unique_ptr")
    {
        ...
        std::unique_ptr<D> extracted_deleter = holder().template extract_deleter<T, D> (context, tinfo->get_memory_guarded_delete);
    ...
    std::unique_ptr<T, D> load_as_const_unique_ptr (const type_info* tinfo,
                                                    T* raw_type_ptr,
                                                    const char* context = "load_as_const_unique_ptr")
    {
        if (! have_holder())
        {
            return unique_with_deleter<T, D> (nullptr, std::unique_ptr<D>());
...
public:
    ...
    explicit type_caster_generic (const type_info* typeinfo)
        : typeinfo (typeinfo)
        , cpptype (typeinfo ? typeinfo->cpptype : nullptr)
    {
    }
...
{
    static constexpr bool is_recursive = container_mapped_type_traits<Container>::has_recursive_mapped_type
