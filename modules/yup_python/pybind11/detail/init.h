                ...
                bool /*need_alias*/)
{
    auto* ptr = unq_ptr.get();
    no_nullptr (ptr);
    auto smhldr = init_smart_holder_from_unique_ptr (std::move (unq_ptr), /*void_cast_raw_ptr*/ true);
    v_h.value_ptr() = ptr;
    v_h.type->init_instance (v_h.inst, &smhldr);
}
                                ...
                                bool need_alias)
{
    ...
    auto smhldr = smart_holder::from_shared_ptr (std::const_pointer_cast<Cpp<Class>> (std::move (shd_ptr)));
...
struct factory<Func, void_type (*)(), Return (Args...)>
{
    ...
    factory (Func&& f)
        : class_factory (std::forward<Func> (f))
    {
    }
...
struct pickle_factory<Get, Set, RetState (Self), NewInstance (ArgState)>
{
    ...
    pickle_factory (Get get, Set set)
        : get (std::forward<Get> (get))
        , set (std::forward<Set> (set))
    {
    }
