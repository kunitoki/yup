...
public:
    ...
    thread_specific_storage (const thread_specific_storage&) = delete;
    thread_specific_storage (thread_specific_storage&&) = delete;
    thread_specific_storage& operator= (const thread_specific_storage&) = delete;
...
public:
    ...
    internals_pp_manager (const char* id, on_fetch_function* on_fetch)
        : holder_id_ (id)
        , on_fetch_ (on_fetch)
    {
    }
...
private:
    std::unique_ptr<InternalsType>* get_or_create_pp_in_state_dict()
    {
        ...
        auto internals_obj = reinterpret_steal<object> (dict_getitemstringref (state_dict.ptr(), holder_id_));
...
inline internals_pp_manager<local_internals>& get_local_internals_pp_manager()
{
    ...
    static const std::string this_module_idstr = PYBIND11_MODULE_LOCAL_ID
