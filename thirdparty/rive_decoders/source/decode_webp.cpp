...
std::unique_ptr<Bitmap> DecodeWebP(const uint8_t bytes[], size_t byteCount)
{
    ...
    {
        fprintf(stderr,
                "DecodeWebP - Input file doesn't appear to be WebP format.\n");
    }
    ...
    std::unique_ptr<uint8_t[]> pixelBuffer =
        std::make_unique<uint8_t[]>(pixelBufferSize);
    ...
    if (WebPDecode(currentFrame.fragment.bytes,
                   currentFrame.fragment.size,
