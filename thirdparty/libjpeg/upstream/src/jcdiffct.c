...
METHODDEF(boolean) compress_diff_data(j_compress_ptr cinfo,
...
start_pass_diff(j_compress_ptr cinfo, J_BUF_MODE pass_mode)
{
  ...
  case JBUF_PASS_THRU:
    ...
    diff->pub._compress_data = compress_diff_data;
    break;
#ifdef FULL_SAMP_BUFFER_SUPPORTED
...
METHODDEF(boolean)
compress_diff_data(j_compress_ptr cinfo, _JSAMPIMAGE input_buf)
{
  my_diff_ptr diff = (my_diff_ptr)cinfo->coef;
...
compress_output(j_compress_ptr cinfo, _JSAMPIMAGE input_buf)
{
  ...
  return compress_diff_data(cinfo, buffer);
}
