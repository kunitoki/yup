...
#include <asmjit/core/ra_local_p.h>
...
static void RAPass_cleanup_after_function(BaseRAPass* pass) noexcept {
  ...
  pass->_work_regs_of_group.for_each([](axl::ArenaVector<RAWorkReg*>& regs) { regs.reset(); });
...
Error BaseRAPass::run(axl::Arena& arena, Logger* logger) {
...
Error BaseRAPass::run_on_function(axl::Arena& arena, FuncNode* func, [[maybe_unused]] bool last) noexcept {
...
Error BaseRAPass::init_shared_assignments(Span<uint32_t> shared_assignments_map) noexcept {
  ...
  for (RABlock* block : _blocks) {
    if (block->has_shared_assignment_id()) {
      ...
      count = axl::max(count, shared_assignment_id + 1);
    }
  }
...
Error BaseRAPass::build_cfg_views() noexcept {
  ...
  axl::ArenaVector<uint32_t> indexes;
...
Error BaseRAPass::build_cfg_dominators() noexcept {
  ...
  while (changed) {
    ...
    for (RABlock* block : _pov.reverse_view()) {
      ...
      for (RABlock* p : predecessors.reverse_view()) {
        if (!p->idom()) {
          continue;
        }
...
Error BaseRAPass::_as_work_reg(RAWorkReg** out, VirtReg* virt_reg) noexcept {
  ...
  axl::ArenaVector<RAWorkReg*>& work_regs_by_group = work_regs(group);
  ...
  if (!virt_reg->is_stack_area()) {
    work_reg->set_reg_byte_mask(axl::trailing_bits_run<uint64_t>(virt_reg->virt_size()));
  }
  ...
  _max_work_reg_name_size = axl::max(_max_work_reg_name_size, virt_reg->name_size());
...
static ASMJIT_INLINE void BaseRAPass_calculate_initial_in_out(RABlock* block, size_t live_word_count, axl::InplaceStack<RABlock*>& queue, uint32_t pov_index) noexcept {
  using BitWord = axl::BitWord;
  ...
  if (!successors.is_empty()) {
    ...
    for (RABlock* successor : successors.reverse_view()) {
      if (axl::bool_and(successor->pov_index() > pov_index, !successor->is_enqueued())) {
...
template<typename BitMutator>
static ASMJIT_INLINE axl::BitWord BaseRAPass_recalculateInOut(RABlock* block, size_t live_word_count, RABlock* successor) noexcept {
  using BitWord = axl::BitWord;
...
static ASMJIT_INLINE Error BaseRAPass_calculateInOutKill(
  ...
  Span<RABlock*> pov = pass->_pov.span_mut();

  size_t multi_work_reg_count = pass->multi_work_reg_count();
  size_t multi_work_reg_count_as_bit_words = axl::BitOps::size_in_words<BitWord>(multi_work_reg_count);
    ...
    for (;;) {
      if (node->is_inst()) {
        ...
        for (uint32_t j = 0; j < count; j++) {
          ...
          if (work_reg->is_within_single_basic_block()) {
            bool was_kill = !axl::test(work_reg->flags(), kLiveFlag);
            bool was_last = !axl::test(work_reg->flags(), kVisitedFlag);

            tied_flags |= axl::bool_as_flag<RATiedFlags::kKill>(was_kill);
            tied_flags |= axl::bool_as_flag<RATiedFlags::kLast>(was_last);

            work_reg->add_flags(kVisitedFlag);
            work_reg->xor_flags(axl::bool_as_flag<kLiveFlag>(uint32_t(is_kill ^ was_kill)));
          }
          else {
  ...
  if (multi_work_reg_count_as_bit_words > 0u) {
    axl::ArenaVector<RABlock*> queue_storage;
    ASMJIT_PROPAGATE(queue_storage.reserve_fit(pass->arena(), pov.size()));
    axl::InplaceStack<RABlock*> queue(queue_storage.data(), pov.size());
    ...
    while (!queue.is_empty()) {
      ...
      for (RABlock* predecessor : block->predecessors()) {
        axl::BitWord changed = BaseRAPass_recalculateInOut<BitMutator>(predecessor, multi_work_reg_count_as_bit_words, block);
        if (axl::bool_and(changed, !predecessor->is_enqueued())) {
...
ASMJIT_FAVOR_SPEED Error BaseRAPass::build_liveness() noexcept {
  ...
  axl::ArenaVector<uint32_t> n_uses_per_work_reg; // Number of USEs of each RAWorkReg.
  axl::ArenaVector<uint32_t> n_outs_per_work_reg; // Number of OUTs of each RAWorkReg.
  axl::ArenaVector<uint32_t> n_insts_per_block;  // Number of instructions of each RABlock.
...

  {
    ...
    if (multi_work_reg_count > 0u && multi_work_reg_count <= axl::bit_size_of<BitWord>) {
      ...
      ASMJIT_PROPAGATE(
        BaseRAPass_calculateInOutKill<axl::BitWordMutator>(
      ...
      ASMJIT_PROPAGATE(
        BaseRAPass_calculateInOutKill<axl::BitVectorMutator>(
          this, n_uses_per_work_reg.data(), n_outs_per_work_reg.data(), n_insts_per_block.data(), Out(num_visits)
        )
      );
    }
  ...
  for (uint32_t i = 0; i < num_all_blocks; i++) {
    ...
    // Process LIVE-IN.
    axl::BitVectorIterator<BitWord> it(block->live_in());
    while (it.has_next()) {
    ...
    for (;;) {
      if (node->is_inst()) {
        ...
        for (uint32_t j = 0; j < count; j++) {
          ...
          // KILL here.
          if (tied_reg->is_last() && (size_t(work_id) >= multi_work_reg_count || !axl::BitOps::bit_at(live_out, work_id))) {
          ...
          if (tied_reg->has_use_id()) {
            ...
            work_reg->add_use_id_mask(axl::bit_mask<RegMask>(use_id));
            if (!work_reg->has_hint_reg_id() && !axl::bit_test(ra_inst->_clobbered_regs[group], use_id)) {
        ...
        position += 2;
        max_live_count.op<axl::Max>(ra_inst->_live_count);
      }
    ...
    _global_live_max_count.op<axl::Max>(max_live_count);
...
Error BaseRAPass::assign_arg_index_to_work_regs() noexcept {
  ...
  for (uint32_t arg_index = 0; arg_index < arg_count; arg_index++) {
    for (uint32_t value_index = 0; value_index < Globals::kMaxValuePack; value_index++) {
      ...
      RAWorkId work_id = work_reg->work_id();
      if (uint32_t(work_id) >= multi_work_reg_count || !axl::BitOps::bit_at(live_in, work_id)) {
        continue;
      }
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    ...
    if (phys_count) {
      ...
      for (size_t phys_id = 0; phys_id < phys_count; phys_id++) {
        new(axl::PlacementNew{&live_spans[phys_id]}) RALiveSpans();
      }
    }
...
ASMJIT_FAVOR_SPEED Error BaseRAPass::bin_pack(RegGroup group) noexcept {
  ...
  ASMJIT_RA_LOG_FORMAT("[bin_pack] Available=%u (0x%08X) Count=%u RegGroup=%u\n",
    axl::popcnt(_available_regs[group]),
  ...
  axl::ArenaVector<RAWorkReg*> work_regs;
  axl::ArenaVector<RAConsecutiveReg> consecutive_regs;
  RALiveSpans tmp_spans;

  axl::ArenaVector<RAWorkReg*>& group_regs = this->work_regs(group);
  ASMJIT_PROPAGATE(work_regs.reserve_fit(arena(), group_regs.size()));

  work_regs.assign_unchecked(group_regs);
  axl::sort(work_regs.span_mut(), [](const RAWorkReg* a, const RAWorkReg* b) noexcept -> bool {
  ...
  if (!work_regs.is_empty()) {
    ...
    for (uint32_t index = 0; index < num_work_regs; index++) {
      ...
      if (work_reg->has_hint_reg_id()) {
        ...
        if (axl::bit_test(available_regs, phys_id)) {
  ...
  if (!consecutive_regs.is_empty()) {
    ...
    for (size_t i = 0;;) {
      ...
      while (i < stop) {
        ...
        if (work_reg->has_immediate_consecutives()) {
          axl::ArenaBitSet::ForEachBitSet it(work_reg->immediate_consecutives());
          while (it.has_next()) {
    ...
    for (RAConsecutiveReg& consecutive_reg : consecutive_regs) {
      ...
      if (!parent_reg) {
        ...
        if (consecutive_id > 31 || !axl::bit_test(available_regs, consecutive_id)) {
          return make_error(Error::kConsecutiveRegsAllocation);
        }

        work_reg->set_hint_reg_id(consecutive_id);
        phys_regs = axl::bit_mask<uint32_t>(consecutive_id);
      }

      while (phys_regs) {
        uint32_t phys_id = axl::bit_size_of<RegMask> - 1 - axl::clz(phys_regs);
...
Error BaseRAPass::run_local_allocator() noexcept {
  ...
  _clobbered_regs.op<axl::Or>(lra._clobbered_regs);
  return Error::kOk;
}
...
Error BaseRAPass::set_block_entry_assignment(RABlock* block, const RABlock* from_block, const RAAssignment& from_assignment) noexcept {
  ...
  {
    axl::BitVectorOpIterator<BitWord, axl::AndNot> it(live_out, live_in);
    while (it.has_next()) {
...
Error BaseRAPass::set_shared_assignment(uint32_t shared_assignment_id, const RAAssignment& from_assignment) noexcept {
  ...
  Span<BitWord> shared_live_in = _shared_assignments[shared_assignment_id]._live_in.span_mut();
  axl::InplaceArray<uint32_t, Globals::kNumVirtGroups> shared_assigned {};
  ...
  for (RABlock* block : blocks()) {
    if (block->shared_assignment_id() == shared_assignment_id) {
      ...
      Span<const BitWord> live_in = block->live_in();
      axl::BitOps::or_(shared_live_in, shared_live_in, live_in);

      for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
        shared_assigned[group] |= entry_phys_to_work_map->assigned[group];

        uint32_t phys_base_index = _phys_reg_index.get(group);
        axl::BitWordIterator<RegMask> it(entry_phys_to_work_map->assigned[group]);
        ...
        while (it.has_next()) {
          ...
          if (!axl::BitOps::bit_at(live_in, work_id)) {
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    uint32_t phys_base_index = _phys_reg_index.get(group);
    axl::BitWordIterator<RegMask> it(_available_regs[group] & ~shared_assigned[group]);

    while (it.has_next()) {
      uint32_t phys_id = it.next();
      if (axl::bit_test(phys_to_work_map->assigned[group], phys_id)) {
...
Error BaseRAPass::block_entry_assigned(const PhysToWorkMap* phys_to_work_map) noexcept {
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    ...
    uint32_t phys_base_index = _phys_reg_index.get(group);
    axl::BitWordIterator<RegMask> it(phys_to_work_map->assigned[group]);
    ...
    while (it.has_next()) {
      ...
      RAWorkReg* work_reg = work_reg_by_id(work_id);
      work_reg->add_allocated_mask(axl::bit_mask<RegMask>(phys_id));
    }
  }
...
Error BaseRAPass::use_temporary_mem(BaseMem& out, uint32_t size, uint32_t alignment) noexcept {
  ...
  if (_temporary_mem.is_none()) {
    ...
    cc().set_stack_size(virt_id, axl::max(virt_reg->virt_size(), size),
                               axl::max(virt_reg->alignment(), alignment));
  }
...
Error BaseRAPass::_mark_stack_args_to_keep() noexcept {
  ...
  axl::ArenaVector<RAWorkReg*>& work_regs = _work_regs;
  size_t num_work_regs = work_reg_count();
...
ASMJIT_FAVOR_SIZE Error BaseRAPass::dump_block_liveness(String& sb, const RABlock* block) noexcept {
  for (uint32_t live_type = 0; live_type < RABlock::kLiveCount; live_type++) {
    ...
    axl::BitVectorIterator<BitWord> it(block->live_bits(live_type));
    if (it.has_next()) {
      bool first = true;
...
ASMJIT_FAVOR_SIZE Error BaseRAPass::dump_live_spans(String& sb) noexcept {
  ...
  for (RAWorkReg* work_reg : _work_regs) {
    RALiveStats& stats = work_reg->live_stats();
