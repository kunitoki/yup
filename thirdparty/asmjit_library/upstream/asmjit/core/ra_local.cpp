      ...
      for (uint32_t value_index = 0; value_index < Globals::kMaxValuePack; value_index++) {
        ...
        if (iter == 0) {
          ...
          if (work_reg->has_home_reg_id()) {
            ...
            if (axl::bit_test(allocable_regs, phys_id)) {
          ...
          if (allocable_regs) {
            uint32_t phys_id = axl::ctz(allocable_regs);
            _cur_assignment.assign(group, work_id, phys_id, true);
            _pass._args_assignment.assign_reg_in_pack(arg_index, value_index, work_reg->type(), phys_id, work_reg->type_id());
          }
          else {
...
Error RALocalAllocator::switch_to_assignment(PhysToWorkMap* dst_phys_to_work_map, Span<const BitWord> live_in, bool dst_is_read_only, bool try_mode) noexcept {
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    ...
    if (!try_mode) {
      axl::BitWordIterator<RegMask> it(cur.assigned(group));
      while (it.has_next()) {
    ...
    while (affected_regs) {
      ...
      axl::BitWordIterator<RegMask> it(affected_regs);
      while (it.has_next()) {
        uint32_t phys_id = it.next();
        RegMask phys_mask = axl::bit_mask<RegMask>(phys_id);
            ...
            if (_arch_traits->has_inst_reg_swap(group)) {
              ...
              if (!cur.is_phys_dirty(group, phys_id)) {
                ...
                if (allocable_regs) {
                  ...
                  uint32_t tmp_phys_id = axl::ctz(allocable_regs);

                  ASMJIT_PROPAGATE(on_move_reg(group, cur_work_reg, cur_work_id, tmp_phys_id, phys_id));
                  _clobbered_regs[group] |= axl::bit_mask<RegMask>(tmp_phys_id);
                }
                else {
...
Cleared:
          ...
          uint32_t alt_phys_id = cur.work_to_phys_id(group, dst_work_id);
          if (alt_phys_id == RAAssignment::kPhysNone) {
            if (axl::BitOps::bit_at(live_in, dst_work_id)) {
    ...
    {
      axl::BitWordIterator<RegMask> it(will_load_regs);
      while (it.has_next()) {
        uint32_t phys_id = it.next();
        ...
        if (!cur.is_phys_assigned(group, phys_id)) {
          ...
          ASMJIT_ASSERT(axl::BitOps::bit_at(live_in, work_id) == true);
...
Error RALocalAllocator::spill_scratch_gp_regs_before_entry(RegMask scratch_regs) noexcept {
  ...
  axl::BitWordIterator<RegMask> it(scratch_regs);
  ...
  for (RegGroup group : axl::enumerate(RegGroup::kMaxVirt)) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (tied_reg->has_any_consecutive_flag()) {
        ...
        if (ASMJIT_UNLIKELY(axl::bit_test(consecutive_mask, consecutive_offset))) {
          return make_error(Error::kInvalidState);
        }

        consecutive_mask |= axl::bit_mask<uint32_t>(consecutive_offset);
      ...
      if (tied_reg->has_use_id()) {
        ...
        RegMask use_mask = axl::bit_mask<RegMask>(tied_reg->use_id());
        ...
        if (assigned_id != RAAssignment::kPhysNone) {
          RegMask assigned_mask = axl::bit_mask<RegMask>(assigned_id);
    ...
    if (consecutive_mask) {
      ...
      consecutive_count = axl::ctz(~consecutive_mask);
      ...
      if (lead->is_use_consecutive()) {
        ...
        axl::BitWordIterator<uint32_t> it(lead->use_reg_mask());
        while (it.has_next()) {
          uint32_t reg_index = it.next();
          if (axl::bit_test(lead->use_reg_mask(), reg_index)) {
            uint32_t score = 15;

            for (i = 0; i < consecutive_count; i++) {
              uint32_t consecutive_index = reg_index + i;
              if (!axl::bit_test(allocable_regs, consecutive_index)) {
                score = 0;
                break;
              }
        ...
        for (i = 0; i < consecutive_count; i++) {
          ...
          RegMask use_mask = axl::bit_mask<uint32_t>(consecutive_index);
    ...
    if (use_pending_count) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (!rm_allocated && tied_reg->has_use_rm()) {
          if (assigned_id == RAAssignment::kPhysNone && axl::is_power_of_2(tied_reg->use_rewrite_mask())) {
            uint32_t op_index = axl::ctz(tied_reg->use_rewrite_mask()) / uint32_t(sizeof(Operand) / sizeof(uint32_t));
        ...
        if (!tied_reg->has_use_id()) {
          ...
          RegMask use_mask = axl::bit_mask<RegMask>(use_id);
          will_use |= use_mask;
    ...
    if (will_free) {
      ...
      axl::BitWordIterator<RegMask> it(will_free);
        ...
        if (_cur_assignment.is_phys_assigned(group, assigned_id)) {
          ...
          if (allocable_regs) {
            ...
            if (reassigned_id != RAAssignment::kPhysNone) {
              ...
              allocable_regs ^= axl::bit_mask<RegMask>(reassigned_id);
              _clobbered_regs[group] |= axl::bit_mask<RegMask>(reassigned_id);
              continue;
            }
          }
        ...
        for (i = 0; i < count; i++) {
          ...
          if (target_work_id != kBadWorkId) {
            ...
            if (available_regs) {
              ...
              _clobbered_regs[group] |= axl::bit_mask<RegMask>(tmp_reg_id);
    ...
    if (out_tied_count) {
      for (i = 0; i < out_tied_count; i++) {
        ...
        if (phys_id != RAAssignment::kPhysNone) {
          _unassign_reg(group, work_id, phys_id);
          will_out &= ~axl::bit_mask<RegMask>(phys_id);
        }
    ...
    // ------
    //
    // Move or spill registers that will be CLOBBERed. Since OUT and KILL were
    // already processed this is used mostly to handle function CALLs.
    //
    // Upstream spilled unconditionally here, which round-trips every loop-carried
    // value through the stack at each per-sample call. Try to move a live value
    // into a register this instruction does not clobber (a preserved one, given
    // that `will_out` covers all scratch registers of the call) when one is free
    // first; spill only when none is. Local change over upstream AsmJit.

    if (will_out) {
      RegMask allocable_regs = _available_regs[group] & ~(_cur_assignment.assigned(group) | will_use | will_out);
      axl::BitWordIterator<RegMask> it(will_out);
      ...
      do {
        ...
        if (allocable_regs) {
          uint32_t reassigned_id = decide_on_reassignment(group, work_reg, phys_id, allocable_regs, ra_inst);
          if (reassigned_id != RAAssignment::kPhysNone) {
            ASMJIT_PROPAGATE(on_move_reg(group, work_reg, work_id, reassigned_id, phys_id));
            allocable_regs ^= axl::bit_mask<RegMask>(reassigned_id);
            _clobbered_regs[group] |= axl::bit_mask<RegMask>(reassigned_id);
            continue;
          }
    ...
    for (i = 0; i < dup_tied_count; i++) {
      ...
      axl::BitWordIterator<RegMask> it(tied_reg->use_reg_mask());
      while (it.has_next()) {
    ...
    if (node->is_invoke() && group == RegGroup::kVec) {
      ...
      if (maybe_clobbered_regs) {
        ...
        axl::BitWordIterator<RegMask> it(maybe_clobbered_regs);
    ...
    if (out_pending_count) {
      ...
      if (consecutive_count) {
        ...
        if (lead->is_out_consecutive()) {
          ...
          axl::BitWordIterator<uint32_t> it(lead->out_reg_mask());
          while (it.has_next()) {
            uint32_t reg_index = it.next();
            if (axl::bit_test(lead->out_reg_mask(), reg_index)) {
              uint32_t score = 15;
      ...
      for (i = 0; i < out_tied_count; i++) {
        ...
        out_regs |= axl::bit_mask<RegMask>(phys_id);
        live_regs &= ~axl::bit_mask<RegMask>(phys_id);
        out_pending_count--;
      }
...
uint32_t RALocalAllocator::decide_on_reassignment(RegGroup group, RAWorkReg* work_reg, uint32_t phys_id, RegMask allocable_regs, RAInst* ra_inst) const noexcept {
  ASMJIT_ASSERT(allocable_regs != 0);
  axl::maybe_unused(phys_id);

  // Prefer reassignment back to HomeId, if possible.
  if (work_reg->has_home_reg_id()) {
    if (axl::bit_test(allocable_regs, work_reg->home_reg_id())) {
  ...
  if (tied_reg && tied_reg->is_out_or_kill()) {
    return axl::ctz(allocable_regs);
  }

  // Prefer reassignment to a register that survives the calls in the value's
  // remaining live range, when one is free. Upstream restricted this to values
  // used within a single basic block; loop-carried values (scalar state, stream
  // pointers, loop invariants) live across a whole sample loop, so they always
  // fell through to the spill below even though a preserved register was free
  // at the call site - and a per-sample libm call then round-tripped every
  // live value through the stack on each iteration. Moving such a value once
  // into a preserved register (or any register this instruction does not
  // clobber) is cheaper than that store + load, and the allocator reconciles
  // the change with the block successors. Local change over upstream AsmJit.
...
uint32_t RALocalAllocator::decide_on_spill_for(RegGroup group, RAWorkReg* work_reg, RegMask spillable_regs, RAWorkId* spill_work_id) const noexcept {
  ...
  axl::maybe_unused(work_reg);
  ASMJIT_ASSERT(spillable_regs != 0);

  axl::BitWordIterator<RegMask> it(spillable_regs);
