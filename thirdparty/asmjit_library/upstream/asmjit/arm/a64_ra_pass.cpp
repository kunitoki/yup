...
#include <asmjit/arm/a64_emit_helper_p.h>
#include <asmjit/arm/a64_inst_api_p.h>
#include <asmjit/arm/a64_inst_db_p.h>
#include <asmjit/arm/a64_ra_pass_p.h>
...
static inline RATiedFlags ra_mem_base_rw_flags(OpRWFlags flags) noexcept {
  constexpr uint32_t shift = axl::ctz(OpRWFlags::kMemBaseRW);
...
static inline RATiedFlags ra_mem_index_rw_flags(OpRWFlags flags) noexcept {
  constexpr uint32_t shift = axl::ctz(OpRWFlags::kMemIndexRW);
  return ra_use_out_flags_from_rw_flags(OpRWFlags(uint32_t(flags) >> shift) & OpRWFlags::kRW);
}
// a64::RACFGBuilder
// =================
...
Error RACFGBuilder::on_instruction(InstNode* inst, InstControlFlow& control_type, RAInstBuilder& ib) noexcept {
  ...
  if (Inst::is_defined_id(inst->real_id())) {
    ...
    ASMJIT_PROPAGATE(InstInternal::query_rw_info(inst->base_inst(), operands.data(), operands.size(), &rw_info));
        ...
        if (op.is_reg()) {
          ...
          if (virt_index < Operand::kVirtIdCount) {
            ...
            if (op_rw_info.consecutive_lead_count()) {
              ...
              if (axl::test(flags, RATiedFlags::kUse)) {
            ...
            if (axl::test(flags, RATiedFlags::kUse)) {
              use_rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&reg._base_id));
              ...
              out_rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&reg._base_id));
              if (op_rw_info.has_op_flag(OpRWFlags::kRegPhysId)) {
            ...
            if (axl::test(flags, RATiedFlags::kLeadConsecutive | RATiedFlags::kUseConsecutive | RATiedFlags::kOutConsecutive)) {
          ...
          if (mem.is_reg_home()) {
            ...
            if (virt_index < Operand::kVirtIdCount) {
              ...
              if (axl::test(flags, RATiedFlags::kUse)) {
                use_rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&mem._base_id));
              }
              else {
                out_rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&mem._base_id));
              }
          ...
          if (mem.has_index_reg()) {
            ...
            if (virt_index < Operand::kVirtIdCount) {
              ...
              if (axl::test(flags, RATiedFlags::kUse)) {
                use_rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&mem._data[Operand::kDataMemIndexId]));
              }
              else {
                out_rewrite_mask = axl::bit_mask<uint32_t>(inst->_get_rewrite_index(&mem._data[Operand::kDataMemIndexId]));
              }
...
void ARMRAPass::on_init() noexcept {
  ...
  _available_regs[RegGroup::kGp] = axl::trailing_bits_run<uint32_t>(_phys_reg_count.get(RegGroup::kGp));
  _available_regs[RegGroup::kVec] = axl::trailing_bits_run<uint32_t>(_phys_reg_count.get(RegGroup::kVec));
  _available_regs[RegGroup::kMask] = axl::trailing_bits_run<uint32_t>(_phys_reg_count.get(RegGroup::kMask));
  _available_regs[RegGroup::kExtra] = axl::trailing_bits_run<uint32_t>(_phys_reg_count.get(RegGroup::kExtra));
      ...
      if (node->is_inst()) {
        ...
        if (ra_inst) {
          ...
          for (uint32_t i = 0; i < tied_count; i++) {
            ...
            axl::BitWordIterator<uint32_t> use_it(tied_reg.use_rewrite_mask());
            if (use_it.has_next()) {
            ...
            axl::BitWordIterator<uint32_t> out_it(tied_reg.out_rewrite_mask());
            if (out_it.has_next()) {
        ...
        if (inst->real_id() == Inst::kIdAdr && operands.size() == 2 && operands[1].is_mem()) {
          ...
          if (!mem.has_base_or_index()) {
            ...
            uint64_t abs_offset = offset < 0 ? axl::neg(uint64_t(offset)) : uint64_t(offset);
...
Error ARMRAPass::update_stack_frame() noexcept {
  if (_func->frame().has_func_calls()) {
    _func->frame().add_dirty_regs(RegGroup::kGp, axl::bit_mask<RegMask>(Gp::kIdLr));
  }
