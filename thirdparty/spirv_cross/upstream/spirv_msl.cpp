                                                            ...
                                                            InterfaceBlockMeta &meta)
{
	...
	if (location_meta)
	{
		...
		if (var.storage == StorageClassInput)
		{
			entry_func.fixup_hooks_in.push_back([=, this, &type, &var]() {
				if (!type.array.empty())
				{
					uint32_t array_size = to_array_size_literal(type);
		...
		{
			entry_func.fixup_hooks_out.push_back([=, this, &type, &var]() {
				if (!type.array.empty())
				{
					uint32_t array_size = to_array_size_literal(type);
					for (uint32_t loc_off = 0; loc_off < array_size; loc_off++)
                                                        ...
                                                        SPIRType &ib_type, SPIRVariable &var, InterfaceBlockMeta &meta)
{
	...
	if (padded_output || padded_input)
	{
		...
		if (padded_output)
		{
			entry_func.fixup_hooks_out.push_back([=, this, &var]() {
	...
	if (var.storage == StorageClassOutput && var.initializer != ID(0))
	{
		if (padded_output || padded_input)
		{
			entry_func.fixup_hooks_in.push_back(
			    [=, this, &var]() { statement(to_name(var.self), " = ", to_expression(var.initializer), ";"); });
		}
		else
		{
			if (meta.strip_array)
		...
		if (var.storage == StorageClassInput && is_builtin_variable(var) && active_input_builtins.get(bi_type))
		{
			...
			case BuiltInTessCoord:
				...
				if (msl_options.tess_domain_origin_lower_left && !is_tessellating_triangles())
				{
					...
					entry_func.fixup_hooks_in.push_back([=, this]() { statement(tc, ".y = 1.0 - ", tc, ".y;"); });
				}
				break;
			case BuiltInSubgroupId:
				if (!msl_options.emulate_subgroups)
			...
			case BuiltInSubgroupLocalInvocationId:
				...
				entry_func.fixup_hooks_in.push_back(
				    [=, this]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = 0;"); });
				break;
			case BuiltInSubgroupSize:
				if (msl_options.emulate_subgroups)
				{
					// For subgroup emulation, assume subgroups of size 1.
					entry_func.fixup_hooks_in.push_back(
					    [=, this]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = 1;"); });
				}
